...
private char *       __hash_get(HASH this, const char *fmt, va_list ap);
 ...
 * This is a local function called by hash_put 
 ...
 */
void
hash_put(HASH this, char *key, char *value)
{
  int  x;
  NODE *node;
 ...
 */
char *
hash_get(HASH this, const char *fmt, ...)
{
  char *ret;
...
char **
hash_get_keys_delim(HASH this, char delim)
{
  int  x;
  int  i = 0;
  char **keys;
  ...
  this->count = 0;
  keys = calloc(this->entries, sizeof(char *));

  for (x = 0; x < this->size; x ++) {
    for(node = this->table[x]; node != NULL; node = node->next){
      tmp = strchr(node->key, delim);
      if (!tmp) {
        keys[i] = (char*)malloc(128);
        memset(keys[i], 0, (char)sizeof(keys[i]));
...
private char *
__hash_get(HASH this, const char *fmt, va_list ap)
{
  int  x;
  NODE *node;
  char key[1024];
...
main()
{
  ...
  for (i = 0; i < len; i++) {
    hash_put(H, simpsons[i][0], simpsons[i][1]);
  }
